...
$select2 = $bdd->query('SELECT * FROM YDA_USERS WHERE USR_ID_PRO=1');
    $idx = 0;
    while ($query = $select2->fetch()){
    
        $admin[$idx]['mail'] = $query['USR_MAIL'];
        $admin[$idx]['name'] = $query['USR_FIRST_NAME'];
        $admin[$idx]['lastName'] = $query['USR_NAME'];
        $idx = $idx + 1;
    }
