...
$("#alertModif").hide();
$("#confirmModif").hide();
$("#alertAdminModif").hide();
$("#confirmAdminModif").hide();
$("#alertProfilModif").hide();
$("#confirmProfilModif").hide();
		...
		function(retour) {
			if (retour == 'WELCOME') {
				$.post("ajax/checkPassword.php", {
					...
					function(retour) {
					   // console.log(retour[1].indexOf('\\?') =! -1)
						if (retour[0] == 'OK') {
						    if(document.location.search.indexOf('redirect') == 1){
						        var url = document.location.search.split('=');
						        document.location.href = url[1];
						      //  if (url[1].includes('?')){
						      //      document.location.href = url[1] + '&token=' + retour[2] + '&id=' + retour[3];
						      //  }else{
						      //      document.location.href = url[1] + '?token=' + retour[2] + '&id=' + retour[3];
						      //  }
						    }else{
						      //  if (retour[1].includes('?')){
						      //      document.location.href = retour[1] + '&token=' + retour[2] + '&id=' + retour[3];
						      //  }else{
						      //      document.location.href = retour[1] + '?token=' + retour[2] + '&id=' + retour[3];
						      //  }
							...
							$("#alertSlideUp").html("Votre mot de passe est erronné. Merci de vérifier.");
							$("#alertSlideUp").fadeTo(3000, 500).slideUp(500, function() {
								$("#alertSlideUp").slideUp(500);
							});
				...
				$("#alertSlideUp").html("Votre compte est en cours de validation par l'administrateur.");
				$("#alertSlideUp").fadeTo(3000, 500).slideUp(500, function() {
					$("#alertSlideUp").slideUp(500);
				});
			}
			else if (retour == 'DUNNO') {
				$("#alertSlideUp").html("Votre adresse mail n'est pas connue. Merci de créer un compte.");
				$("#alertSlideUp").fadeTo(3000, 500).slideUp(500, function() {
					$("#alertSlideUp").slideUp(500);
				});
			}
			else if (retour == 'NOPE') {
				$("#alertSlideUp").html("Votre compte a été désactivé. S'il s'agit d'une erreur, merci de vous rapprocher de l'administrateur.");
				$("#alertSlideUp").fadeTo(3000, 500).slideUp(500, function() {
					$("#alertSlideUp").slideUp(500);
				});
			}
...
$(document).keypress(function(e) {
	...
	if (keycode == '13') {
		if (window.location.pathname == '/yoda/login.php' && !document.getElementById('createModal').classList.contains('show') && !document.getElementById('forgetModal').classList.contains('show')) {
			$("#login").click();
		}
		else if (window.location.pathname == '/yoda/login.php' && document.getElementById('createModal').classList.contains('show') && !document.getElementById('forgetModal').classList.contains('show')) {
			$("#createAccount").click();
		}
		else if (window.location.pathname == '/yoda/login.php' && !document.getElementById('createModal').classList.contains('show') && document.getElementById('forgetModal').classList.contains('show')) {
			$("#resetPassword").click();
...
$("#createAccount").click(function(evt) {
	...
	if (flag === 0) {
		...
		$.post("ajax/checkUser.php", {
			...
			function(retour) {
				if (retour == 'WELCOME') {
					$("#alertCreate").html("L'adresse mail renseignée existe déja. Merci de vous connecter normalement.");
					...
					$("#alertCreate").html("L'adresse mail renseignée concerne un compte désactivé. S'il s'agit d'une erreur, merci de contacter l'administrateur.");
					$("#alertCreate").fadeTo(3000, 500).slideUp(500, function() {
						$("#alertCreate").slideUp(500);
					});
				}
				else if (retour == 'WAIT') {
					$("#alertCreate").html("Votre compte est en cours de validation par l'administrateur.");
					$("#alertCreate").fadeTo(3000, 500).slideUp(500, function() {
						$("#alertCreate").slideUp(500);
					});
					...
					$.post("ajax/createUser.php", {
						...
						function(retour) {
							if (retour['ok'] == 'ok') {
								$("#confirmCreate").html("Le compte a bien été créé. Une demande a été envoyée à l'administrateur. Vous recevrez une notification dès que l'accès sera autorisé.");
								$("#confirmCreate").fadeTo(3000, 500).slideUp(3000, function() {
									$("#confirmCreate").slideUp(3000);
		...
		function(retour) {
			if (retour == 'WELCOME' || retour == 'WAIT') {
				$.post("ajax/resetPassword.php", {
					...
					function(retour) {
						if (retour == 'ok') {
							$("#confirmReset").html("Votre mot de passe a bien été réinitialisé. Il a été envoyé sur votre adresse mail.");
							$("#confirmReset").fadeTo(3000, 500).slideUp(500, function() {
								$("#confirmReset").slideUp(500);
				...
				$("#alertReset").html("L'adresse mail renseignée concerne un compte désactivé. S'il s'agit d'une erreur, merci de contacter l'administrateur.");
				$("#alertReset").fadeTo(3000, 500).slideUp(500, function() {
					$("#alertReset").slideUp(500);
				});

			}
			else if (retour == 'DUNNO') {
				$("#alertReset").html("Votre adresse mail n'est pas connue. Merci de créer un compte.");
				$("#alertReset").fadeTo(3000, 500).slideUp(500, function() {
					$("#alertReset").slideUp(500);
				});
			}
