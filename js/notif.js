...
$("#alertNotif").hide();
$("#confirmNotif").hide();
$("#alertNotifConfig").hide();
$("#confirmNotifConfig").hide();
$("#alertNotifUser").hide();
$("#successNotifUser").hide();
...
$("#updateNotif").click(function(evt) {
    ...
    $.ajax({
    		...
    		if (retour == 'ok') {
    			$("#confirmNotif").html("Les modifications apportées ont été sauvegardées.");
    			$("#confirmNotif").fadeTo(3000, 500).slideUp(500, function() {
    				$("#confirmNotif").slideUp(500);
    			});
    		}
		});
});
...
//   console.log(document.getElementsByClassName('selected'));
   if(search == ''){
        ...
        $.get("ajax/search.php?search=" + search +"&filter=none", function(json){
...
function selectProfil(id){
    ...
    if (document.getElementById('profil-'+id).classList.contains("list-group-item-light")){
        ...
        // console.log(document.getElementById('profil-'+id).classList);
...
function selectUser(id){
    ...
    if (document.getElementById('user-'+id).classList.contains("list-group-item-light")){
        ...
        var profilOK = 1;
        $(".userNotif").each(function(){
            // console.log($(this).data('profil-id'));
...
$('#sendMailButton').click(function(){
    ...
    if(flag == 1){
        ...
        $.ajax({
    		...
    		if (retour == 'ok') {
    			$("#successNotifUser").html("La notification a été envoyée.");
    			$("#successNotifUser").fadeTo(3000, 500).slideUp(500, function() {
    				$("#successNotifUser").slideUp(500);
    			});
    		}
		});
		...
		function(retour) {
			...
			for (var idx = 0; idx < arrayLength; idx++) {
				...
				document.getElementById('dropDownUser').innerHTML = document.getElementById('dropDownUser').innerHTML + "<li class='dropdown-item dropdownUserLi' id='user-" + retour[idx]['id'] + "' onclick='selectUserNotif(" + retour[idx]['id'] + ")' style='color:" + retour[idx]['color'] + ";'>" +
					ucFirst(retour[idx]['name']) + " " + ucFirst(retour[idx]['lastName']) + " - " + ucFirst(retour[idx]['profil']) + "</li>";
			}
		});
});
...
$("#updateNotifConfig").click(function(evt) {
    ...
    $.ajax({
    		...
    		if (retour == 'ok') {
    			$("#confirmNotifConfig").html("Les modifications apportées ont été sauvegardées.");
    			$("#confirmNotifConfig").fadeTo(3000, 500).slideUp(500, function() {
    				$("#confirmNotifConfig").slideUp(500);
    			});
    		}
		});
});